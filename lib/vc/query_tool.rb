module Vc
  class QueryTool < Client
    ...
    def get_disaster_query(query_id, params={}, format=:xls)
      raise "Unknown query format #{format.inspect}" unless [:xls, :pdf, :csv].include? format
    ...
    def set_incident_id id
      # https://volunteerconnection.redcross.org/?html=xmlhttp-vms_incident_selector&incident_selector=181&rand=0.53908036728123
      self.get '/', query: {html: 'xmlhttp-vms_incident_selector', incident_selector: id, rand: SecureRandom.random_number}
    end
